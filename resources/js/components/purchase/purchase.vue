<template>
    <div>
        ...
        <div aria-hidden="true" aria-labelledby="exampleModalLabel"
             :class="showSearchTypeModal?'modal fade show d-block modaldrg':'modal fade modaldrg'" role="dialog"
             tabindex="-1">
                ...
                <div class="modal-content m-auto" style="min-width: 850px;">
                    ...
                    <div class="modal-body min-h-50" style="min-height:500px;overflow: auto">
                        <input v-model="typeName" class="form-control-sm"
                               ...
                               type="search"
                        >
                        <button class="btn btn-sm btn-info" @click="findTypeByLike()"><i class="fa fa-search font-weight-bold text-light"></i></button>
                        <table style="position: absolute; z-index: 999;overflow: auto"
                               class="text-center ">
                            <thead>
                            <tr>
                            ...
                            <tr v-for="searchType in searchTypes">
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-secondary" data-dismiss="modal"
                                @click="searchTypes=[];typeName='';showSearchTypeModal=false"
                                type="button">إغلاق
                        </button>
                    </div>
                </div>
            ...
            <div class="card-header ">
                <form class="w-100 " direction="rtl" @submit.prevent="saveBill">
                    <table class="table-bordered text-lighten-1 text-center font-weight-bold">
                        <tbody>
                        <tr>
                            <td style="width:10%">كود المورد</td>
                            <td style="width:10%"><input v-model="search" class="form-control-sm"
                                                         type="text"
                                                         @keyup="findSupplier"></td>
                            <td style="width:10%">اسم المورد</td>
                            <td style="width:10%"><input :value="supplier.supplier_name"
                                                         class=" form-control-sm "
                                                         placeholder=""
                                                         type="text"></td>
                            <td style="width:10%"><a :title="supplier.supplier_total_remain"
                            ...
                            <td style="width:10%"><select v-model="form.pay" @change="calcTotal()"
                                                          class="form-control form-control-sm float-left text-center">
                                <option v-for="(pay,index) in payMethods" :value="pay.id" :key="index">
                                    {{ pay.pay_method_name }}
                                </option>
                            </select></td>
                        ...
                        <tr>
                            ...
                            <td style="width:10%"><input
                                ...
                                class="form-control form-control-sm  float-left text-center"
                            /></td>
                            <td style="width:10%" v-show="$root.$data.roles.includes(12)">الخصم:{{
                                }}
                            </td>

                            <td style="width:10%" v-show="$root.$data.roles.includes(12)"><input
                            ...
                            <td style="width:100%">
                                <input v-model="form.note" class="form-control-sm"
                                       placeholder="ملاحظات الفاتورة"></input>
                            </td>
                            <td>
                                ...
                                <div id="body" class="col-sm-12 ">
                                    <table
                                        ...
                                        <tr>
                                            ...
                                            <th class="col-header" style="width:30%">العدد</th>
                                            <th style="width: 10%" v-show="$root.$data.mixins.use_type_uints">الوحدة</th>
                                        ...
                                        >
                                            ...
                                            <td>
                                                ...
                                                <a v-else class="btn mx-2 btn-warning"
                                                   @click="decrement(product)">-</a>
                                                <input
                                                ...
                                                <a class="btn mx-2 btn-success"
                                                   @click="increment(product)">+</a>


                                            </td>
                                            <td v-show="$root.$data.mixins.use_type_uints">
                                                <select class="form-control-sm" v-model="product.sell_unit"
                                                        @change="calcTotalTypeCost(product)">
                                                    <option :selected="unit.type_unit_id===product.sell_unit"
                                            ...
                                            <td>
                                                ...
                                                <b class="btn  text-bold ">{{
                                                        Number(product.total_purchase_price).toFixed(2)
                                                    }}</b>
                                            </td>
                                        ...
                                        <tbody v-else="">
                                        <tr  class=" w-100" >
                                            <td colspan="4" class="m-auto font-weight-light" >لا توجد أصناف</td>
                                        </tr>
                                        </tbody>
                            ...
                            <div class="col-lg-12 mb-4">
                                <div class="table-responsive w-100">
                                    ...
                                    <table class="table align-items-center table-flush">
                                        ...
                                        <tr>
                                            <th class="col-header" style="width:10%">كود المورد</th>
                                            <th class="col-header" style="width:30%">اسم المورد</th>
                                            <th class="col-header" style="width:30%">هاتف المورد</th>
                                            <th class="col-header" style="width:10%">رصيد المورد</th>
                                        ...
                                        <tr v-for="supplier in filterSearch">
                                            ...
                                            <td><button class="btn btn-success btn-sm" data-dismiss="modal" @click="selectSupplier(supplier)"><i
...
export default {
    ...
    filterSearch() {
      return this.suppliers.length > 0
        ? this.suppliers.filter((supplier) => {
            return supplier.supplier_name.match(this.searchName);
          })
        : "";
    },
  },
  ...
  data() {
    return {
      hasRole: this.$root.$data.roles.includes(7),
  ...
  methods: {
    ...
    async removeFromCart(product, index) {
      ...
      product.total_purchase_price =
        product.type_count * Number(product.type_price);
    ...
    async calcUnitPrice(product) {
      if (product.sell_unit !== null) {
        ...
        product.type_price =
          product.type_purchases_price / this.typeUnit.no_of_unit ??
    ...
    async calcTotalTypeCost(product) {
      if (this.$root.$data.mixins.use_type_uints) {
    ...
    async calcSum() {
      ...
      this.cart.filter((product) => {
        this.form.sum += product.total_purchase_price;
      });
    ...
    async saveValidBill() {
      ...
      await axios
        ...
        .catch((error) => {
          Notification.customMsg("error", "topRight", error.message);
        });
    },
    clearAll() {
      this.cart = [];
